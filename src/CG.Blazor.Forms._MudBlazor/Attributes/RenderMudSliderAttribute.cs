            ...
            )
        {
            ...
            try
            {
                ...
                if (null == model)
                {
                    // Let the world know what we're doing.
                    logger.LogDebug(
                        "RenderMudSliderAttribute::Generate called with a null model!"
                        );
                ...
                // We only render MudSlider controls against numerics.
                    ...
                    modelType == typeof(decimal))
                {
                    // Let the world know what we're doing.
                    logger.LogDebug(
                        "Rendering property: '{PropName}' as a MudSlider.",
                        prop.Name
                        );
                    ...
                    // Is the control bound to a byte?
                    if (modelType == typeof(byte))
                    {
                    ...
                    // Is the control bound to an int?
                    ...
                    // Is the control bound to a long?
                    ...
                    // Is the control bound to a float?
                        ...
                        attributes["ValueChanged"] = RuntimeHelpers.TypeCheck<EventCallback<float>>(
                            EventCallback.Factory.Create<float>(
                                ...
                                EventCallback.Factory.CreateInferred<float>(
                                    eventTarget,
                                    x => prop.SetValue(propParent, x),
                                    (float)prop.GetValue(propParent)
                                    )
                                )
                            );
                    ...
                    // Is the control bound to a double?
                        ...
                        attributes["ValueChanged"] = RuntimeHelpers.TypeCheck<EventCallback<double>>(
                            EventCallback.Factory.Create<double>(
                                ...
                                EventCallback.Factory.CreateInferred<double>(
                                    eventTarget,
                                    x => prop.SetValue(propParent, x),
                                    (double)prop.GetValue(propParent)
                                    )
                                )
                            );
                    ...
                    // Is the control bound to a decimal?
                    else 
                    {
                        // Ensure the Value property value is set.
                        ...
                        attributes["ValueChanged"] = RuntimeHelpers.TypeCheck<EventCallback<decimal>>(
                            EventCallback.Factory.Create<decimal>(
                                ...
                                EventCallback.Factory.CreateInferred<decimal>(
                                    eventTarget,
                                    x => prop.SetValue(propParent, x),
                                    (decimal)prop.GetValue(propParent)
                                    )
                                )
                            );
                ...
                else
                {
                    ...
                    logger.LogDebug(
                        ...
                        "because we only render mud slider components on properties " +
            ...
            catch (Exception ex)
            {
                ...
                throw new FormGenerationException(
                    message: "Failed to render a mud slider field! " +
